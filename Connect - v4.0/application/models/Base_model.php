	...
	public function guardar($args=[])
	{
		...
		if ($this->_pk === null) {
			if (empty($this->usuario_id)) {
				if (property_exists($this, 'usuario_id')) {
					$this->usuario_id = verPropiedad($args, "usuario_id", elemento($this->user, "id", NULL));
				}
			}


			if ((property_exists($this, 'usuario_id') && $this->usuario_id === NULL)) {
