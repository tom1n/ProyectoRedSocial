...
class Publicacion extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->ruta = FCPATH . "assets/archivos";
	...
	public function guardar_video($id="")
	{
		if ($this->input->method() === 'post') {
			$data  = ['exito' => 0];
			$datos = (object) $_POST;
			...
			if (elemento($_FILES, 'video')) {
				$nombre = $_FILES['video']['name'];
				$ubicacion = "{$this->ruta}/{$nombre}";

				if (move_uploaded_file($_FILES['video']['tmp_name'], $ubicacion)) {
					$plb->video_enlace = "assets/archivos/{$nombre}";
				}	
			}
